select distinct
    ...
    quote_ident(ni.nspname)||'.'||quote_ident(c.relname) as tag_object_name,
    ('index ' || quote_ident(ni.nspname)||'.'||quote_ident(c.relname) || ' on ' || quote_ident(s.schemaname) || '.' || quote_ident(s.tablename) || ' column ' || quote_ident(s.attname)  || ' could possibly be declared partial leaving out NULL-s')::text as recommendation,
...
from
    ...
    join pg_namespace ni on ni.oid = c.relnamespace
where
  not indisprimary
  and not indisunique
  and indisready
